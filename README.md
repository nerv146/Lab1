...
```mermaid
flowchart TD
    A[Начало] --> B[Ввод a] --> C[Ввод b] --> D{b >= 0?}
    D{b >= 0?} --да--> E[Вывести: Решим неравенство: a*x/(x - b) >= 0] --> F[Вывести: Ограничение: x ≠ b] --> G{a > 0?} 
    G{a > 0?} --да--> H[Вывести: x/(x - b) >= 0] --> I{b != 0?}  
    I{b != 0?} --да--> J[Вывести: x ∈ (-∞; 0] ∪ (b; +∞)]
    I{b != 0?} --нет--> K[Вывести: x ∈ (-∞; 0) ∪ (0; +∞)]
    G{a > 0?} --нет--> L[Вывести: x ∈ (-∞; b) ∪ (b; +∞)]
    L[Вывести: x ∈ (-∞; b) ∪ (b; +∞)] --> N[Вывести: x/(x - b) <= 0] --> O[Вывести: x ∈ [0; b)] 
    O[Вывести: x ∈ [0; b)] -->W[Конец]

    P[Вывести: Нет решений!]
    Q[Вывести: Решим неравенство: a*x/(x + (-b)) >= 0]
    R[Вывести: Ограничение: x ≠ b]
    S[Вывести: x/(x + (-b)) >= 0]
    T[Вывести: x ∈ (-∞; b) ∪ [0; +∞)]
    U[Вывести: x/(x + (-b)) <= 0]
    V[Вывести: x ∈ (b; 0)]
